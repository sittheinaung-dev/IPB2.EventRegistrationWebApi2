...
namespace IPB2.EventRegistration.Domain.Features.Registration
{
    public class RegistrationServices
    {
        ...
        public async Task<RegistrationCreateResponse> RegisterParticipant(RegistrationCreateRequest request)
        {
            try
            {
                ...
                var registrationEntity = new IPB2.EventRegistrationWebApi.Database.AppDbContextModels.Registration
                {
                ...
                var participantName = participantEntity.ParticipantName ?? "";
        ...
        public async Task<RegistrationUpdateResponse> UpdateRegistration(RegistrationUpdateRequest request)
        {
            try
            {
                ...
                string finalStatus = request.Status ?? "Confirmed";
                if (finalStatus == "Confirm") finalStatus = "Confirmed";
                if (finalStatus == "Cancel") finalStatus = "Cancelled";
        ...
        public async Task<List<IPB2.EventRegistrationWebApi.Database.AppDbContextModels.Participant>> GetActiveParticipants()
        {
            return await _context.Participants.Where(x => x.IsDelete == false).ToListAsync();
        }
    }
}
